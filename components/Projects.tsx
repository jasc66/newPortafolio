...
function ProjectCard({ project, index }: { project: Project; index: number }) {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setTimeout(() => {
            setIsVisible(true)
          }, index * 100) // Escalonar la animación basada en el índice
        }
      },
      { threshold: 0.1 },
    )
  ...
  return (
    <a
      ...
      className={`block group relative overflow-hidden rounded-lg bg-light-navy hover:bg-lightest-navy transition-all duration-500 ${
